...
import { buildFillResult, moduleForCanonical, signalToCanonical, suggestionForReason, type FillEntry, type FillResult } from "../../lib/fillResult";
...
export async function fillCurrentForm(dryRun = false): Promise<FillResult> {
  const profile = await getProfile();
  const entries: FillEntry[] = [];
  const review = new Map<string, AutofillReviewItem>();
  const firstFields = extractFields();
  await fillPass(firstFields, profile, review, entries, dryRun);
  ...
  if (revealedFields.length > 0) await fillPass(revealedFields, profile, review, entries, dryRun);
  ...
  for (const field of extractFields()) {
    ...
    if (isLegalConfirmation(field)) {
      entries.push({ label: field.question, canonicalField: "", value: "", status: "skipped", reason: "legal_skip", module: "法律确认", suggestion: suggestionForReason("legal_skip") });
    } else if (target && isManualPageControl(field, target)) {
      entries.push({ label: field.question, canonicalField: "", value: "", status: "custom_component", reason: "custom_component", module: "其他", suggestion: suggestionForReason("custom_component") });
    } else if (target && targetHasValue(target)) {
      entries.push({ label: field.question, canonicalField: "", value: fieldValue(target instanceof Array ? target[0] : target) as string, status: "filled", reason: "ok", module: "其他" });
    }
  }

  const result = buildFillResult(entries);
  if (!dryRun) {
...
async function fillPass(fields: FieldDescriptor[], profile: Profile, review: Map<string, AutofillReviewItem>, entries: FillEntry[], dryRun = false): Promise<void> {
  ...
  for (const field of fieldsToMap) {
    ...
    if (!fill) {
      ...
      const canonical = signalToCanonical(signal);
      entries.push({ label: field.question, canonicalField: canonical ?? "", value: "", status: "no_data", reason: "no_data", module: canonical ? moduleForCanonical(canonical) : "其他", suggestion: suggestionForReason("no_data") });
      continue;
    }
    const result = dryRun ? { ok: true, detail: "dry-run" } : await applyFill(target, fill);
    const canonical = fill.source === "profile" ? signalToCanonical(directFieldSignal(field)) : undefined;
    const status = result.ok ? "filled" : "fill_error";
    const reason = result.ok ? "ok" : "fill_error";
    ...
    entries.push({
      ...
      status,
      reason,
      module: canonical ? moduleForCanonical(canonical) : "其他",
      suggestion: result.ok ? undefined : suggestionForReason("fill_error")
    });
  }
}
