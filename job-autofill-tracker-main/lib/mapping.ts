...
import { loadCustomSynonyms } from "./customSynonyms";
import type { AnswerMemory, CanonicalField, FieldDescriptor, FieldFill, Profile } from "./schema";

export function normalizeText(value: string): string {
  return value.toLowerCase().replace(/[^a-z0-9+\u4e00-\u9fff ]+/g, " ").replace(/\s+/g, " ").trim();
}
...
export async function deterministicValue(field: FieldDescriptor, profile: Profile): Promise<FieldFill | undefined> {
  const normalized = normalizeText(field.question);

  // 统一字典匹配（从 customSynonyms 加载，中英文合并，可通过 UI 配置）
  const entries = await loadCustomSynonyms();
  const dict: Record<string, string[]> = {};
  for (const entry of entries) {
    if (!entry.enabled) continue; // 跳过已关闭的条目
    dict[entry.field] = entry.synonyms;
  }
  const matched = matchSynonym(normalized, dict);
  if (matched) {
    const fill = tryFill(profile, field, matched);
    if (fill) return fill;
  }
...
function matchSynonym(normalized: string, dict: Record<string, string[]>): string | undefined {
  return Object.entries(dict).find(([, patterns]) =>
    patterns.some((pattern) => normalized.includes(pattern))
  )?.[0];
}

function tryFill(profile: Profile, field: FieldDescriptor, canonical: string): FieldFill | undefined {
  const raw = getProfileValue(profile, canonical as CanonicalField);
...
function getProfileValue(profile: Profile, key: CanonicalField): string | boolean | undefined {
  switch (key) {
    ...
    case "identity.fullName":
      return profile.identity.fullName || [profile.identity.firstName, profile.identity.lastName].filter(Boolean).join("");
    case "identity.email":
    ...
    case "profile.summary":
      return profile.summary || profile.additionalKnowledge;
